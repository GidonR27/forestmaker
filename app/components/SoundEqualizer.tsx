        ...
        {Object.entries(sounds).map(([sound, state]) => {
          ...
          const isActive = state.isActive;
            ...
            return (
              ...
              <div 
                className={`relative h-32 md:h-36 w-16 flex items-center justify-center py-4 transition-all duration-300 ${
              ...
              >
                  {/* Invisible touch helper divs at top and bottom */}
                  <div 
                    className="absolute top-0 left-0 w-full h-10 -translate-y-full" 
                    style={{ touchAction: 'none' }}
                    onClick={(e) => {
                      // When clicking the top helper, set slider to maximum
                      e.stopPropagation();
                      setSounds(prev => ({
                        ...prev,
                        [sound as SoundType]: {
                      ...
                      playSoundWithThrottle(sound as SoundType, 1);
                      setSliderActive(sound as SoundType, true);
                      setTimeout(() => setSliderActive(sound as SoundType, false), 3000);
                    }}
                  />
                  <div 
                    className="absolute bottom-0 left-0 w-full h-10 translate-y-full" 
                    style={{ touchAction: 'none' }}
                    onClick={(e) => {
                      // When clicking the bottom helper, set slider to minimum
                      e.stopPropagation();
                      setSounds(prev => ({
                        ...prev,
                        [sound as SoundType]: {
                      ...
                      playSoundWithThrottle(sound as SoundType, 0);
                      setSliderActive(sound as SoundType, true);
                      setTimeout(() => setSliderActive(sound as SoundType, false), 3000);
                    }}
                  />
